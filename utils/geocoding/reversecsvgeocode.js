...
Geocoder.prototype.run = function(input,output,options) {
  ...
  function csvParsed(parsed) {
    ...
    if (options.zipcode === null) {
       options = misc.discoverOptionsReverse(options,parsed[0]);
     }
  ...
  function codeRow(row,cb) {
    ...
    if (cache[url]) {
      ...
      row[options.street] = cache[url].street;
      row[options.city] = cache[url].city;
      row[options.zipcode] = cache[url].zipcode;
      row[options.county] = cache[url].county;
      row[options.state] = cache[url].state;
  ...
  function handleResponse(body,row,url,cb) {
    ...
    //Success
    } else if ("street" in result && "city" in result && "zipcode" in result && "county" in result && "state" in result) {

      row[options.street] = result.street;
      row[options.city] = result.city;
      row[options.zipcode] = result.zipcode;
      row[options.county] = result.county;
      row[options.state] = result.state;
  ...
  function successful(row) {
    return row[options.street] && row[options.city] && row[options.zipcode] && row[options.county] && row[options.state]
  }
